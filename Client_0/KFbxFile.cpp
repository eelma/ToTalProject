	...
	for (int iDraw = 0; iDraw < m_pDrawObjList.size(); iDraw++)
	{
		if (m_pDrawObjList[iDraw]->m_dxMatrixBindPseMap.size())
		{
			...
			pContext->UpdateSubresource(
				m_pDrawObjList[iDraw]->m_pSkinBoneCB, 0, nullptr, &m_cbDataBone, 0, 0);
		}
	}
	return true;
}

void KFbxFile::UpdateFrameA(ID3D11DeviceContext* pContext,
	float fTime, VS_CONSTANT_BONE_BUFFER& cbData)
{
	for (int iBone = 0; iBone < m_pObjectList.size(); iBone++)
	{
...
void KFbxFile::UpdateFrameB(ID3D11DeviceContext* pContext, VS_CONSTANT_BONE_BUFFER& cbInputData,
