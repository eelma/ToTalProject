...
void KGamePlayer::Frame(float fDeltaTime, float fGameTime)
{    
...
void KGamePlayer::Render() {
    std::cout << "Player3D:"
        << m_pImp->m_Box.vMin.x << "," << m_pImp->m_Box.vMin.y << ","
        << m_pImp->m_Box.vMin.z << std::endl;
};
void KGamePlayer2D::Frame(float fDeltaTime, float fGameTime)
{
    m_pImp->m_fSpeed = 30.0f;
    m_pImp->m_fMass = 10.0f;
    ...
    KVector2D linearAcc = m_pImp->m_vAcceleration2D;
