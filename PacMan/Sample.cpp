...
bool  Sample::Init()
{
    ...
    {
    m_pfood = new KFood2D;
    m_pfood->Create(m_pd3dDevice, m_pImmediateContext,
        L"../../data/PacMan.png",
        L"DefaultShapeMask.txt");
    if (i % 2 == 0)
    {
        m_pfood->SetRect({ 272,90,9,9 });
    }
    else
    {
        m_pfood->SetRect({ 268,108,15,15 });
    }
    m_pfood->SetDirection({ randstep(-1.0f, 1.0f),randstep(-1.0f,1.0f) });
    m_pfood->SetPosition({ 100.0f + i * 100.0f,100.0f });
    m_pfood->SetMask(pMaskTex);
    m_pNpcList.push_back(m_pfood);
    }

    /*for(int i=0;i<7;i++)
    {
      KEnemy2D * npc = new KEnemy2D;
...
bool  Sample::Frame()
{
    ...
    for (auto obj : m_pNpcList)
    {
        ...
        if (KCollision::RectToInRect(m_pfood->m_rtUV, m_pUser->m_rtUV))
        {
...
GAME_RUN(KSampleSprite_0, 600, 800)
